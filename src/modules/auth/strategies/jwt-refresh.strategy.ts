...
export class JwtRefreshStrategy extends PassportStrategy(Strategy, 'jwt-refresh') {
  constructor(
    ...
    super({
      ...
      secretOrKey: configService.get<string>('app.jwt.refreshSecret') || 'default-refresh-secret-key',
    });
  }
  ...
  async validate(payload: JwtPayload): Promise<AuthenticatedMerchant> {
    ...
    if (!merchant) {
      throw new UnauthorizedException('Invalid refresh token: merchant not found');
    }
